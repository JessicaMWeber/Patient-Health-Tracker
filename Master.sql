--Create Medicine Database prior to running this

USE Medicine
GO

DROP PROC [AddMedToPatient]
DROP PROC [AddPatient]
DROP PROC [DeletePatient]
DROP PROC [GetMedicationByName]
DROP PROC [GetMedicationsBySymptom]
DROP PROC [GetPatient]
DROP PROC [GetPatientIDS]
DROP PROC [AddMedications]
DROP PROC [LoadPatientandVitals]
DROP PROC [SavePatient]
DROP PROC [SaveVitalSigns]
DROP PROC [GetMedsForPatient]
DROP PROC [GetMedsTakenHistory]
DROP PROC [AddMedsTakenHistory]
DROP PROC [DeleteMedsTakenHistory]
DROP PROC [SaveMedsTakenHistory]
DROP PROC [GetMedsTakenHistoryIDByMedID]
DROP PROC [GetMedsTakenByDate]
DROP PROC [GetMedications]
GO

DROP TABLE [MedsTakenHistory]
...
(
	ID int PRIMARY KEY IDENTITY (1,1),
	MedID int FOREIGN KEY REFERENCES Medications(ID) NOT NULL,
	PatientID int FOREIGN KEY REFERENCES Patient(ID) NOT NULL,
	Dose varchar(50) NOT NULL,
	[Route] varchar(100) NOT NULL,
	MedTime datetime2 NOT NULL
)
GO
...
CREATE TABLE MedsTakenHistory
(
	ID INT PRIMARY KEY IDENTITY (1,1),
	PatientID INT FOREIGN KEY REFERENCES Patient (ID) NOT NULL,
	MedID int FOREIGN KEY REFERENCES Medications (ID) NOT NULL,
	TimeTaken DATETIME2 NOT NULL
)
GO
...
CREATE PROCEDURE AddMedications
@id int,
@patientID int,
@dose VARCHAR(100),
@route VARCHAR(100),
...
BEGIN
	...
	VALUES
	( @id, @patientID, @Dose, @Route, @MedTime )
...
AS
BEGIN
	DELETE FROM MedsTakenHistory
	WHERE PatientID = @ID
	DELETE FROM MedsForPatient
	WHERE PatientID = @ID
...
AS
BEGIN
	SELECT m.ID, m.MedName, m.Symptom, mfp.Dose, mfp.Route, mfp.MedTime FROM MedsForPatient mfp
...
CREATE PROC [GetMedsTakenHistory]
@ID int
AS
BEGIN
	SELECT * FROM MedsTakenHistory
	WHERE ID = @ID
END
GO

CREATE PROC [GetMedications]
@ID INT
AS
BEGIN
	SELECT m.ID,
           ...
           mfp.Dose,
           mfp.Route,
           mfp.MedTime FROM Medications m
	LEFT JOIN MedsForPatient mfp
	ON mfp.MedID = m.ID
	WHERE m.ID = @ID
END
GO

CREATE PROC [AddMedsTakenHistory]
@PatientID INT,
@MedID int,
@TimeTaken DATETIME2
AS
BEGIN
	INSERT INTO MedsTakenHistory
	VALUES
	(   
		@PatientID,
		@MedID, 
...
CREATE PROC [DeleteMedsTakenHistory]
@ID INT
AS
BEGIN
	DELETE FROM MedsTakenHistory
...
CREATE PROC SaveMedsTakenHistory
@ID INT,
@PatientID INT,
@MedID INT,
@TimeTaken DATETIME2
...
CREATE PROC [GetMedsTakenHistoryIDByMedID]
@MedID INT
AS
BEGIN
	SELECT ID FROM MedsTakenHistory
	WHERE MedID = @MedID
END
GO

CREATE PROC [GetMedsTakenByDate]
@PatientID INT,
@Date DATETIME2
AS
BEGIN
